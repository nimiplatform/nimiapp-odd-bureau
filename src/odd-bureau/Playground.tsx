...
import { persistPlay, preparePlay, restorePlay } from './play-runtime.js';
import type { PlayRound, PlaySave } from './play-rules.js';
...
export function Playground() {
  ...
  const file = useRef<HTMLInputElement>(null), photoRef = useRef<Photo | null>(null), saveRef = useRef<PlaySave | null>(null), operation = useRef<AbortController | null>(null), photoRequest = useRef<AbortController | null>(null), writeQueue = useRef(Promise.resolve()), setupBack = useRef<'home' | 'round'>('home');
  ...
  function persist(next: PlaySave, image = photoRef.current) {
    if (!image) return;
  ...
  useEffect(() => {
    ...
    return () => { live = false; operation.current?.abort(); photoRequest.current?.abort(); };
  }, []);
  ...
  async function choose(file: File) {
    if (unavailable || operation.current || photoRequest.current) return;
    ...
    try {
      ...
      replacePhoto(next); saveRef.current = null; setSave(null); setSaveNote(''); setFound([]); setRoute('home');
    } catch (e) { if (!controller.signal.aborted) setError(errorMessage(e)); }
    finally { if (photoRequest.current === controller) { photoRequest.current = null; if (!controller.signal.aborted) setLoading(false); } }
  }
  async function begin() {
    if (!photo || unavailable || photoRequest.current || operation.current) return;
    ...
    try {
      ...
      controller.signal.throwIfAborted(); saveRef.current = next; setSave(next); persist(next, photo); setRoute('round');
    } catch (e) { if (!controller.signal.aborted) setError(errorMessage(e)); }
  ...
  return <div className="odd-bureau playground" data-density="expressive">
    <header className="bureau-header"><button className="wordmark" onClick={() => setRoute('home')} disabled={unavailable} aria-label="回到玩法选择"><Fingerprint size={33} strokeWidth={1.7}/><span>奇物局<span className="wordmark-en">ODD BUREAU</span></span></button><p className="header-motto">让日常，出一点意外。</p><div className="header-actions"><button className="mystery-link" disabled={unavailable} onClick={() => setRoute('mystery')}><Search size={15}/>照片探案</button><IconButton className="help-button" icon={<Settings2 size={20}/>} aria-label="能力设置" disabled={unavailable} onClick={settings}/></div></header>
    {route === 'setup' ? <Setup onBack={() => { void checkConfig(); setRoute(setupBack.current); }}/>
      : route === 'round' && save && photo ? <main className="bureau-main playground-main"><div className="play-heading"><div><span className="activity-name">{save.round.kind === 'machine' ? <AudioLines size={17}/> : <MessageCircle size={17}/>} {save.round.kind === 'machine' ? '怪机器' : '罢工谈判'}</span><h1>{save.round.plan.title}</h1>{save.round.kind === 'machine' && <p>{save.round.plan.invitation}</p>}</div><Button className="quiet-button" leadingIcon={<ArrowLeft size={16}/>} onClick={() => setRoute('home')}>换个玩法</Button></div>
        ...
        <div className={`play-layout playground-lobby ${dragging ? 'drop-ready' : ''}`} onDragOver={e => { e.preventDefault(); if (!unavailable) setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={e => { e.preventDefault(); setDragging(false); if (e.dataTransfer.files[0]) void choose(e.dataTransfer.files[0]); }}>
          ...
          </PhotoStage> : <div className="play-photo-empty"><ImagePlus size={43}/><p>{loading ? '正在打开游乐场…' : '选一张桌面或房间照片，从这里开始。'}</p><Button className="quiet-button" disabled={unavailable} onClick={() => file.current?.click()}>选照片</Button></div>}
          <aside className="activity-panel lobby-panel"><h2>今天，怎么闹？</h2><p className="lobby-description">每件物品都会有自己的本领和脾气。<br/>这次由你来改变局面。</p><div className="activity-choices" role="group" aria-label="选择玩法"><button className={kind === 'machine' ? 'selected' : ''} aria-pressed={kind === 'machine'} disabled={unavailable} onClick={() => setKind('machine')}><AudioLines size={26}/><span><strong>怪机器</strong><b>把滴答装进杯子。</b><small>连起物品，听听你的发明。</small></span><ArrowRight size={18}/></button><button className={kind === 'strike' ? 'selected' : ''} aria-pressed={kind === 'strike'} disabled={unavailable} onClick={() => setKind('strike')}><MessageCircle size={26}/><span><strong>罢工谈判</strong><b>这张桌子不干了。</b><small>谈条件、许承诺，把故事会办起来。</small></span><ArrowRight size={18}/></button></div>
            <div className="lobby-cta"><Button className="primary-key" trailingIcon={<ArrowRight size={19}/>} disabled={unavailable || !photo} onClick={() => void begin()}>{loading ? '正在打开照片…' : busy ? '正在让物品登场…' : !configured ? '接通 AI，准备开玩' : kind === 'machine' ? '用这张照片，造台怪机器' : '用这张照片，谈一场罢工'}</Button><Button className="upload-key" leadingIcon={<ImagePlus size={17}/>} disabled={unavailable} onClick={() => file.current?.click()}>换成我的照片</Button>{save && <button className="resume-play" disabled={unavailable} onClick={() => setRoute('round')}>继续上次的{save.round.kind === 'machine' ? '怪机器' : '故事会'}<ArrowRight size={14}/></button>}</div><p className="lobby-footnote">先用这张试玩照片体验。<br/>自己的杯子、书、灯，也能成为主角。</p>
          </aside>
        </div>
      </main>}
    ...
    <input ref={file} type="file" accept="image/png,image/jpeg,image/webp" hidden disabled={unavailable} aria-label="选择游乐场照片" onChange={e => { const next = e.currentTarget.files?.[0]; e.currentTarget.value = ''; if (next) void choose(next); }}/>
  </div>;
}
